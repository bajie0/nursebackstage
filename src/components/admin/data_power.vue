              ...
              <el-row>
                <el-col :span="6">
                  <div class="ui-container-table-search-quick">
                    ...
                    <el-input v-model="search_input" placeholder="权限标题 / 权限编码..." @input="searchQuick"
                              clearable></el-input>
                  </div>
                </el-col>
                <el-col :span="4">
                      ...
                      <el-select v-model="searchForm.power_parent" placeholder="请选择上级权限" filterable clearable>
                        <div v-for="item_one in power_data" :key="item_one.id">
                          ...
                          <div v-for="item_two in item_one.children" :key="item_two.id">
                            <el-option :key="item_two.id" :value="item_two.id"
                                       :label="'|---- '+item_two.label"></el-option>
                            <div v-for="item_three in item_two.children" :key="item_three.id">
                              <el-option :key="item_three.id" :value="item_three.id"
                                         :label="'|------ '+item_three.label"></el-option>
                              <div v-for="item_four in item_three.children" :key="item_four.id">
                                <el-option :key="item_four.id" :value="item_four.id"
                                           :label="'|-------- '+item_four.label"></el-option>
                                <div v-for="item_five in item_four.children" :key="item_five.id">
                                  <el-option :key="item_five.id" :value="item_five.id"
                                             :label="'|---------- '+item_five.label"></el-option>
                                  <div v-for="item_six in item_five.children" :key="item_six.id">
                                    <el-option :key="item_six.id" :value="item_six.id"
                                               :label="'|------------ '+item_six.label"></el-option>
                                  </div>
                                </div>
                              </div>
                            </div>
            ...
            <div :class="table_list?'ui-container-table-content':'ui-container-table-content ui-none'">
              <div class="ui-container-table-list">
                <el-table :data="tableData" height="100%" style="width: 100%" @selection-change="selection_change"
                  ...
                  <el-table-column prop="power_type" label="权限分类" width="130" :filters="power_type"
                                   :filter-method="typeTag">
                  ...
                  <el-table-column prop="power_state" label="审核状态" width="100" :filters="power_state" :filter-method="stateTag" v-if="page_state===true">
                  ...
                  <div class="ui-table-panel">
                    ...
                    <div class="ui-table-panel-content">
                      <el-form label-position="left" inline class="ui-table-expand">
                        ...
                        <el-form-item label="审核状态" v-if="page_state===true">
                          <span>
                            <el-switch v-model="item.power_state" :active-value="true" :inactive-value="false" active-text inactive-text @change="changeSwitch($event,item.id)"></el-switch>
                          </span>
                        </el-form-item>
                      </el-form>
                    </div>
                    <ul class="ui-table-panel-footer" v-if="page_patch===true || page_delete===true">
                      <li v-if="page_patch===true">
                        <el-link @click="handle_panel_edit(item.id)"><i
                          class="ion-ios-create"></i>编辑
                        </el-link>
                      </li>
                      <li v-if="page_delete===true">
                        <el-link @click="handle_panel_delete(item.id)"><i
                          class="ion-ios-trash"></i>删除
                        </el-link>
                      </li>
                    </ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 09.Table数据 - 编辑操作
      handle_edit(row) {
        window.location.href = "/admin/patch-power/" + row.id
      },
      handle_panel_edit(row) {
        window.location.href = "/admin/patch-power/" + row
      },
      // 10.Table数据 - 删除操作
      handle_delete(row) {
      ...
      blurSort(row) {
        if (row.power_sort === "") {
          ...
          let params = {"id": row.id, "power_sort": row.power_sort}
          ApiState(url, params).then((response) => {
            this.$message.success(response)
          })
        }
