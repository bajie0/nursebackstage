  ...
  <el-container>
      ...
      <el-main>
        <div class="ui-container" v-if="page_get===true">
          <div class="ui-container-header">
            <h2><i class="ion-ios-arrow-forward"></i>全局字典管理</h2>
            <p>对平台的全局字典相关信息进行删除、审核、导入、导出等系统操作。</p>
              ...
              <el-row>
                <el-col :span="6">
                  <div class="ui-container-table-search-quick">
                    ...
                    <el-input v-model="search_input" placeholder="全局字典标题 / 全局字典编码..." @input="searchQuick" clearable></el-input>
                  </div>
                </el-col>
                <el-col :span="4">
                ...
                <el-row :gutter="20">
                  <el-col :span="6">
                    <el-form-item label="上级全局字典" prop="basics_parent">
                      <el-select v-model="searchForm.basics_parent" placeholder="请选择上级全局字典" filterable clearable>
                  ...
                  <el-col :span="6">
                    <el-form-item label="全局字典分类" prop="basics_type">
                      <el-select v-model="searchForm.basics_type" placeholder="请选择全局字典分类" clearable>
                        <el-option label="分类级" value="1"></el-option>
                        <el-option label="数据级" value="2"></el-option>
                      </el-select>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
                    <el-form-item label="全局字典编码" prop="basics_code">
                      <el-input v-model="searchForm.basics_code" placeholder="请选择全局字典编码"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
                    <el-form-item label="全局字典标题" prop="basics_title">
                      <el-input v-model="searchForm.basics_title" placeholder="请选择全局字典标题"></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
                    <el-form-item label="审核状态" prop="basics_state">
                      <el-select v-model="searchForm.basics_state" placeholder="请选择审核状态" clearable>
                  ...
                  <el-table-column type="expand" width="40">
                    <template slot-scope="props">
                      <el-form label-position="left" inline class="ui-table-expand">
                        <el-form-item label="全局字典分类">
                          <div v-if="props.row.basics_type === 1">
                            <el-tag type="primary" disable-transitions>分类级</el-tag>
                          </div>
                          <div v-if="props.row.basics_type === 2">
                            <el-tag type="success" disable-transitions>数据级</el-tag>
                          </div>
                        </el-form-item>
                        <el-form-item label="上级全局字典">
                          <span>{{ props.row.basics_parent }}</span>
                        </el-form-item>
                        <el-form-item label="全局字典编码">
                          <span>{{ props.row.basics_code }}</span>
                        </el-form-item>
                        <el-form-item label="全局字典标题">
                          <span>{{ props.row.basics_title }}</span>
                        </el-form-item>
                      </el-form>
                    </template>
                  </el-table-column>
                  <el-table-column prop="basics_type" label="全局字典分类" width="130" :filters="basics_type" :filter-method="typeTag">
                    <template slot-scope="basics_type">
                      <div v-if="basics_type.row.basics_type === 1">
                        <el-tag type="primary" disable-transitions>分类级</el-tag>
                      </div>
                      <div v-if="basics_type.row.basics_type === 2">
                        <el-tag type="success" disable-transitions>数据级</el-tag>
                      </div>
                    </template>
                  </el-table-column>
                  <el-table-column prop="basics_parent" label="上级全局字典" width="200"></el-table-column>
                  <el-table-column prop="basics_code" label="全局字典编码" width="300" sortable></el-table-column>
                  <el-table-column prop="basics_title" label="全局字典标题" width="500"></el-table-column>
                  <el-table-column prop="basics_state" label="审核状态" width="100" :filters="basics_state" :filter-method="stateTag" v-if="page_state===true">
                    <template slot-scope="basics_state">
                      <el-switch v-model="basics_state.row.basics_state" :active-value="true" :inactive-value="false" active-text inactive-text @change="changeSwitch($event,basics_state.row.id)"></el-switch>
                    </template>
                  </el-table-column>
                ...
                <el-col :span="8" v-for="(item,index) in tableData" :key="index" v-else>
                  <div class="ui-table-panel">
                    <div class="ui-table-panel-header">
                      <h3><i class="ion-ios-arrow-forward"></i>{{item.basics_title}}</h3>
                    </div>
                    <div class="ui-table-panel-content">
                      <el-form label-position="left" inline class="ui-table-expand">
                        <el-form-item label="全局字典分类">
                          <div v-if="item.basics_type === 1">
                            <el-tag type="primary" disable-transitions>分类级</el-tag>
                          </div>
                          <div v-if="item.basics_type === 2">
                            <el-tag type="success" disable-transitions>数据级</el-tag>
                          </div>
                        </el-form-item>
                        <el-form-item label="上级全局字典">
                          <span>{{ item.basics_parent }}</span>
                        </el-form-item>
                        <el-form-item label="全局字典编码">
                          <span>{{ item.basics_code }}</span>
                        </el-form-item>
                        <el-form-item label="审核状态" v-if="page_state===true">
                          <span>
                            <el-switch v-model="item.basics_state" :active-value="true" :inactive-value="false" active-text inactive-text @change="changeSwitch($event,item.id)"></el-switch>
                          </span>
                        </el-form-item>
                      </el-form>
                    </div>
    ...
    <el-drawer class="ui-upload" title="批量导入数据" :visible.sync="upload_all" :direction="upload_all_direction" :with-header="false" :before-close="uploadClose">
      ...
      <div class="ui-upload-content">
        <h4>01.模板</h4>
        <a href="../../../static/template/全局字典模板.xlsx">
  ...
  export default {
    data() {
      return {
        ...
        page_size: 20,
        // 权限分类筛选
        basics_type: [{text: '分类级', value: 1}, {text: '数据级', value: 2}],
        // 审核筛选
        basics_state: [{text: '通过', value: true}, {text: '待审', value: false}],
        // 上级权限
        basics_data: [],
      }
    },
    ...
    methods: {
      ...
      // 09.Table数据 - 编辑操作
      handle_edit(row) {
        window.location.href = "/config/patch-basics/" + row.id
      },
      handle_panel_edit(row) {
        window.location.href = "/config/patch-basics/" + row
      },
      // 10.Table数据 - 删除操作
      handle_delete(row) {
      ...
      // 12.打开新增页面
      operate_add() {
        this.$router.push("/config/add-basics")
      },
      // 13.批量上传
      operate_upload() {
        this.upload_all = true
        this.upload_content = ""
        this.upload_url = this.$api_url + "/api/upload/basics/"
      },
      ...
      operate_download(command) {
        if (command === 'operate_download_all') {
          this.$confirm("是否导出全部数据？", "系统提示", {
            ...
            let url = "/api/download/basics_all/"
        ...
        if (command === 'operate_download_part') {
          if (this.multipleTable.length === 0) {
            ...
            this.$confirm("是否导出所选数据？", "系统提示", {
              ...
              let url = "/api/download/basics_part/"
      ...
      // 17.全局字典分类 - 筛选
      typeTag(value, row) {
        return row.basics_type === value;
      },
      // 18.审核状态 - 筛选
      stateTag(value, row) {
        return row.basics_state === value;
      },
      // 19.引入全局字典json
      getBasicsJson() {
      ...
      // 20.页面权限验证
      getPageAuthority() {
        if (this.$store.state.power_list.indexOf("HandleManageAdminBasicsCreate") !== -1) {
          this.page_create = true
        }
        if (this.$store.state.power_list.indexOf("HandleManageAdminBasicsGet") !== -1) {
          this.page_get = true
        }
        if (this.$store.state.power_list.indexOf("HandleManageAdminBasicsPatch") !== -1) {
          this.page_patch = true
        }
        if (this.$store.state.power_list.indexOf("HandleManageAdminBasicsDelete") !== -1) {
          this.page_delete = true
        }
        if (this.$store.state.power_list.indexOf("HandleManageAdminBasicsState") !== -1) {
          this.page_state = true
        }
        if (this.$store.state.power_list.indexOf("HandleManageAdminBasicsUpload") !== -1) {
          this.page_upload = true
        }
        if (this.$store.state.power_list.indexOf("HandleManageAdminBasicsDownload") !== -1) {
